...
export const getConfigurationOptions = function (
    query,
    defaultName,
    extension,
    geoStoreBase
    ...
    if (mapId) {
        configUrl =
            (geoStoreBase || defaultConfig.geoStoreUrl) + "data/" + mapId
    } else {
        configUrl =
            defaultConfig.configurationFolder +
            (query.config || defaultName || "config") +
            "." +
            (extension || "json")
    }
    return {
...
export const filterUrlParams = (urlToFilter, params = []) => {
    ...
    return getUrlWithoutParameters(
        cleanDuplicatedQuestionMarks(urlToFilter),
        params
    )
}
        ...
        if (
            ...
            return url.format(
                assign(
                    {},
                    parsed,
                    { search: null, pathname: newPathname },
                    {
                        query: assign(
                            {
                                service: "WPS",
    ...
    if (localConfigFile) {
        ...
        return axios
            ...
            .then((configs) => {
                ...
                const merged = mergeConfigsPatch(
                    main,
                    patches.filter((c) => c && typeof c === "object")
                )
...
export const getCenter = function (center, projection) {
    ...
    const transformed =
        crs !== "EPSG:4326"
            ? Proj4js.transform(new Proj4js.Proj(crs), epsg4326, point)
            : point
...
export const replacePlaceholders = function (inputUrl) {
    ...
    ;(currentUrl.match(/\{.*?\}/g) || []).forEach((placeholder) => {
        const replacement =
            defaultConfig[placeholder.substring(1, placeholder.length - 1)]
...
export const normalizeConfig = function (config) {
    ...
    return {
        map: other,
        layers: layers
            .map(setApiKeys, config)
            .map(setLayerId)
            .map(setUrlPlaceholders),
        groups: groups,
        plugins: plugins,
    }
}

export const getUserConfiguration = function (
    defaultName,
    extension,
    geoStoreBase
) {
...
export const getProxiedUrl = function (uri, config = {}) {
    ...
    if (!sameOrigin) {
        ...
        if (proxyUrl) {
            ...
            const isCORS = useCORS.reduce(
                (found, current) => found || uri.indexOf(current) === 0,
                false
            )
            if (!isCORS) {
