...
export const getLocale = function (query = {}) {
    ...
    const defaultLocale = supportedLocales.en
        ? { key: "en", locale: supportedLocales.en }
        : { key, locale: supportedLocales[key] }
    let locale =
        supportedLocales[
            LocaleUtils.normalizeLocaleCode(
                query.locale ||
                    (navigator
...
export const getErrorMessage = (e, service, section) => {
    return (
        (service &&
            section &&
            errorParser[service] &&
            errorParser[service][section] &&
            errorParser[service][section](e)) || {
 ...
 */
export const getLocalizedProp = (locale, prop) =>
    isObject(prop) ? prop[locale] || prop.default : prop || ""
